 ...
 * @param {string|null} userEmail — the applicant's email
 * @param {object} [options]
 * @param {string} [options.notificationTarget] — per-prototype override for
 *   where this notification should go. Falls back to MDA env default if
 *   absent or empty. Set by the generator UI's per-file Options panel and
 *   stored in prototypes/<folder>/meta.json.
 */
async function sendNotification(formName, formData, referenceNumber, userEmail, options) {
  const target = (options && options.notificationTarget && String(options.notificationTarget).trim()) || MDA;
  try {
    const result = await sendEmail(
      target,
    ...
    console.log(`  ✓ MDA notification sent to ${target} (id: ${result.messageId})`);
    return result;
  } catch (err) {
